...
interface TransactionDao: PagingAndSortingRepository<Transaction, Long> {
    ...
    @Query("select T from Transaction T where T.raider.id = :raiderId")
    fun findAllByRaiderId(@Param("raiderId") raiderId: Long): List<Transaction>

    @Query("select T from Transaction T where" +
            "(:raiderId = null OR T.raider.id = :raiderId)")
    fun findAllTest(@Param("raiderId") raiderId: Long?): List<Transaction>
